  ...
  while (keys.length > 0) {
    ...
    if (io) {
      ...
      _emitToTeam(io, tid, 'brackets-msg', {
        ...
        opponentScore: [scores[otid], match.scores[teamIndex ? 0 : 1]]
      }); 
    }
  ...
  for (let match of matchups) {
    ...
    if (match.scores[0] !== match.scores[1]) {
      ...
      if (match.tb && match.tb[0] !== match.tb[1]) {
        ...
        // Higher seed wins if TB is also equal
        winnerIndex = match.seeds.indexOf(Math.max(...match.seeds)); 
      }
    }
