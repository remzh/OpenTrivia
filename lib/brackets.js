...
function generateBrackets(num, data) {
  ...
  for (let match of data) {
    ...
    if (typeof match.bracket !== 'undefined' && typeof match.index !== 'undefined') {
 ...
 * @param {object} scores - object w/ team ids as keys (from question.scores)
 * @param {object} 
 */
async function updateScores(io, db, round, scores, tb) {
  ...
  while (keys.length > 0) {
    ...
    let teamIndex = match.seeds[0] === matchData.teamSeed ? 0 : 1; 
    match.scores[teamIndex] += scores[tid]; 
    if (tb && tb[tid]) {
      if (!match.tb) {
        match.tb = [0, 0]; 
      }
      match.tb[teamIndex] += tb[tid]; 
    }

    let otid = false; 
    if (matchData.opponent) {
      otid = matchData.opponent.t; 
      if (typeof scores[otid] !== 'undefined') {   
        match.scores[teamIndex ? 0 : 1] += scores[otid]; 
        if (tb && tb[otid]) {
          if (!match.tb) {
            match.tb = [0, 0]; 
          }
          match.tb[otid] += tb[otid]; 
        }
        keys.splice(keys.indexOf(otid), 1); // Remove opponent TID to prevent duplicate
      } else {
        scores[otid] = 0; 
      }
      if (io) {
        // Send score update to opponent team
        _emitToTeam(io, otid, 'brackets-msg', {
          type: 'scoreUpdate', 
          teamScore: [scores[otid], match.scores[teamIndex ? 0 : 1]], 
    ...
    if (io) {
      ...
      _emitToTeam(io, tid, 'brackets-msg', {
        ...
        opponentScore: otid ? [scores[otid], match.scores[teamIndex ? 0 : 1]] : [0, 0]
      }); 
    }
...
/**
 * Emits "brackets-endMatch" events to all teams, "finalizes" results, and builds the brackets for next round based on results. 
 * @param {object} io - socket.io instance
 * @param {array} teams - list of all teams from metadata object
 * @param {array} matchups - list of all matchups to send out
 * @param {*} roundNum - which round to finalize
 */
async function finishMatchups(io, db, roundNum) {
  ...
  for (let match of matchups) {
    ...
    if (match.scores[0] !== match.scores[1]) {
      winnerIndex = match.scores.indexOf(Math.max(...match.scores)); 
    } else {
      // Tie -- first see if there's tiebreaker points, if not, the higher seed will win
      if (match.tb && match.tb[0] !== match.tb[1]) {
        winnerIndex = match.tb.indexOf(Math.max(...match.tb)); 
      } else {
        // Higher seeded team wins if TB is also equal
        winnerIndex = match.seeds.indexOf(Math.min(...match.seeds)); 
      }
    }
    // let winnerSeed = match.seeds[winnerIndex]; 
    let loserIndex = winnerIndex ? 0 : 1; 
    // let loserSeed = match.seeds[loserIndex]; 
    
    let teams = [metadata.seeds[match.seeds[0]], metadata.seeds[match.seeds[1]]]; 
    
    // Write the matchup into the next round
    if (match.w && match.l) {
      // console.log(match.w, match.l); 
      // console.log({
      //   round: roundNum + 1, 
      //   bracket: match.bracket,  // bracket is the same
      //   game: match.w[0]
      // }); 
      let res = await db.updateOne({
        ...
        $set: {
          [`seeds.${match.w[1]}`]: match.seeds[winnerIndex]
        }
      }); 
      db.updateOne({
        round: roundNum + 1, 
        ...
        $set: {
          [`seeds.${match.l[1]}`]: match.seeds[loserIndex]
        }
      }); 
    }
    ...
    for (let i = 0; i < 2; i++) {
      if (teams[i]) {
        let team = teams[i]; 
        _emitToTeam(io, team.t, 'brackets-endMatch', {
          score: match.scores[i], 
          opponentScore: match.scores[i?0:1], 
          winner: i === winnerIndex
        }); 
        // console.log('broadcast:', team.t, i === winnerIndex); 
        // Send broadcasts to both teams with the result
      }
    }
  }
  return count; 
...
async function routeMessage(io, db, params, message) {
  ...
  return {ok: true, sent: true, otid: opponentTID}; 
}
